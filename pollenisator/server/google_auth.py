...
def google_callback() -> Union[Any, ErrorStatus]:
    ...
    try:
        ...
        if user_record is None:
            # Auto-create user if they don't exist (you may want to restrict this)
            # Option 1: Return error and require admin to create user first
            logger.warning(f"User with email {email} not found in database")
            return "User not found. Please contact administrator to create your account.", 403
            
            # Option 2: Auto-create user (uncomment if you want this behavior)
            # username = email.split('@')[0]  # Use email prefix as username
            # # Check if username already exists
            # existing_user = dbclient.findInDb("pollenisator", "users", {"username": username}, False)
            # if existing_user is not None:
            #     username = f"{username}_{google_user_id[:8]}"  # Make unique
            # 
            # user_data = {
            #     "username": username,
            #     "email": email,
