...
def getIpPortsNmap(pentest, nmapFile, keep_only_open=True):
    """
    Read the given nmap .nmap file results and return a PluginResult with ips and ports.
        Args:
            pentest: the pentest name
            nmapFile:  the path to the .xml file generated by an nmap scan
            keep_only_open: if True, only add IPs that have open ports
...
class NmapXML(Plugin):
    ...
    def Parse(self, pentest, file_opened, **kwargs):
        ...
        if cmdline is not None:
            if " -sP " in cmdline:
                keep_only_open = False
        result = getIpPortsNmap(pentest, file_opened, keep_only_open)
        if result is None:
            return PluginResult.empty()
        result.tags = tags
        result.lvl = "scope"
