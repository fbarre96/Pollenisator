...
def collectBloodHoundResults(pentest, users, computers, result):
    """Collect BloodHound results into a PluginResult."""
    inserted_user = 0
    inserted_computer = 0
    for computer in computers:
        result.ips.append(Ip(pentest).initialize(str(computer["ip"]), infos={"plugin": BloodHound.get_name()}))
...
class BloodHound(Plugin):
    ...
    def Parse(self, pentest: str, file_opened: IO[bytes], **kwargs: Dict[str, Any]) -> PluginResult:
        ...
        inserted_user, inserted_computer = collectBloodHoundResults(pentest, users, computers, result)
